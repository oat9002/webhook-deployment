...
object AuthenticationMiddleware {
  ...
  private val onFailure: AuthedRoutes[String, IO] = Kleisli(req => OptionT.pure(Response[IO](Status.Unauthorized)))
  val apply: AuthMiddleware[IO, Boolean] = AuthMiddleware(validateApiKey, onFailure)
}